...
ulight_lang ulight_get_lang(const char *name,
                            size_t name_length) ULIGHT_NOEXCEPT;

#ifdef ULIGHT_HAS_CHAR8
/// @brief Like `ulight_get_lang`, but using `char8_t` instead of `char`.
ulight_lang ulight_get_lang_u8(const char8_t *name,
                               size_t name_length) ULIGHT_NOEXCEPT;
#endif
...
ulight_lang ulight_lang_from_path(const char *path,
                                  size_t path_length) ULIGHT_NOEXCEPT;

#ifdef ULIGHT_HAS_CHAR8
/// @brief Like `ulight_get_lang_of_path`, but using `char8_t` instead of
/// `char`.
ulight_lang ulight_lang_from_path_u8(const char8_t *path,
                                     size_t path_length) ULIGHT_NOEXCEPT;
#endif
...
/// @brief An array of `ulight_lang_entry` containing the list of languages
/// supported by ulight. The entries are ordered lexicographically by `name`.
...
///
/// The display name is a human-readable name for the language, and may contain
/// spaces. For example, `ulight_lang_display_name(ULIGHT_LANG_CPP)` is `"C++"`.
...
ulight_u8string_view
ulight_lang_display_name_u8(ulight_lang lang) ULIGHT_NOEXCEPT;
#endif
...
typedef enum ulight_status {
  ...
  /// @brief Something else is wrong with the `ulight_state` that isn't
  /// described by one of
  /// the
  /// above.
  ...
  /// @brief Something went wrong that is not described by any of the other
  /// statuses.
...
typedef enum ulight_flag {
  ...
  /// For example, two adjacent operators (e.g. `+`) with `ULIGHT_HL_SYM_OP`
  /// would get combined
  /// into a single one.
  ///
  /// While this produces more compact output,
  /// it prevents certain forms of styling from functioning correctly.
  /// For example, if we ultimately produce HTML and want to put a border around
  /// each operator,
  ...
  /// For example, the `<<` operator in C++ results in one `ULIGHT_HL_SYM_OP`
  /// token,
  /// but `{}` results in two `ULIGHT_HL_SYM_BRACE` tokens.
  ULIGHT_COALESCE = 1,
  /// @brief Adhere as strictly to the most recent language specification as
  /// possible.
  /// For example, this means that C keywords or compiler extension keywords in
  /// C++
  /// do not get highlighted.
  ///
  /// By default, ulight "over-extends" its highlighting a bit to provide better
  /// UX,
...
typedef enum ulight_highlight_type {
  ...
  /// @brief In coding languages,
  /// an identifier in the declaration of a module, namespace, or other such
  /// construct.
  ...
  /// like the leading character of `'label` in Rust, `@label` in Kotlin,
  /// `.label` in NASM, etc.
  ...
  /// That is, an instruction like `db` which doesn't correspond to any CPU
  /// instruction,
  ...
  /// @brief The declaration of a new shell command or function in a build
  /// system.
  ...
  /// such as `span` in HTML, which is a builtin tag unlike some custom tag like
  /// `my-tag`.
  ULIGHT_HL_MARKUP_TAG_BUILTIN = 0xb2,
  /// @brief In Markup languages, the delimiter of a tag, like `<` or `>` in
  /// `<html>`.
  ...
  /// @brief In languages where a symbol has special meaning, a special symbol
  /// in general.
  ULIGHT_HL_SYMBOL = 0xe0,
  /// @brief  Punctuation such as commas and semicolons that separate other
  /// content,
  /// and which are of no great significance.
  /// For example, this includes commas and semicolons in C,
  /// but does not include commas in Markup languages, where they have no
  /// special meaning.
...
ULIGHT_DEPRECATED
ulight_string_view
ulight_highlight_type_id(ulight_highlight_type type) ULIGHT_NOEXCEPT;
...
/// The returned `ulight_string_view` is null-terminated.
ulight_string_view
ulight_highlight_type_long_string(ulight_highlight_type type) ULIGHT_NOEXCEPT;

#ifdef ULIGHT_HAS_CHAR8
/// @brief Like `ulight_highlight_type_long_string`,
/// but returns `ulight_u8string_view`.
ulight_u8string_view ulight_highlight_type_long_string_u8(
    ulight_highlight_type type) ULIGHT_NOEXCEPT;
#endif

/// @brief Returns a textual representation made of ASCII characters and
/// underscores of `type`. This is used as a value in `ulight_tokens_to_html`.
/// The returned `ulight_string_view` is null-terminated.
ulight_string_view
ulight_highlight_type_short_string(ulight_highlight_type type) ULIGHT_NOEXCEPT;

#ifdef ULIGHT_HAS_CHAR8
/// @brief Like `ulight_highlight_type_short_string`,
/// but returns `ulight_u8string_view`.
ulight_u8string_view ulight_highlight_type_short_string_u8(
    ulight_highlight_type type) ULIGHT_NOEXCEPT;
#endif

typedef struct ulight_token {
  /// @brief The index of the first code point within the source code that has
  /// the highlighting.
  size_t begin;
...
typedef struct ulight_state {
  ...
  /// is invoked with `flush_tokens_data`, `token_buffer`, and
  /// `token_buffer_length`.
  ...
  /// is invoked with `flush_text_data`, `text_buffer`, and
  /// `text_buffer_length`.
...
/// It is the caller's responsibility to store the tokens permanently if they
/// want to, such as in a `std::vector` in C++.
...
/// Additionally, the user has to provide a token buffer for intermediate
/// conversions.
///
/// The token buffer is pointed to by `state->token_buffer`
